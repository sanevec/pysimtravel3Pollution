...
def calculate_total_pollution(data, pollutant,prefix):
    ...
    for key in data.keys():
        if prefix == 'A':
            if pollutant in key and 'G' in key:
...
def load_and_generate_maps(prefix, params):
    ...
    if prefix == 'A':
        output_dir = 'representaciones2'
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)

        pollutant='CO2'        
        data=calculate_total_pollution(data,pollutant,prefix)
        # Crear un mapa de color para cada clave
        plt.figure(figsize=(10, 8))
        plt.imshow(np.transpose(data), cmap='viridis')
        plt.colorbar()
        plt.title(f'{prefix} - {pollutant} - {params["seed"]}')
        print('here')
        # Guardar la figura
        output_filename = f"{prefix}_{pollutant}_delta_{params['delta']}_gamma_{params['gamma']}_times_{params['times']}_seed_{params['seed']}_buildings_{params['buildings']}_distributionCS_{params['distributionCS']}_densityCars_{params['densityCars']}_densityEV_{params['densityEV']}_densityDiesel_{params['densityDiesel']}_windV_{params['windV']}_pollutionRouting_{params['pollutionRouting']}.png"
