...
output_path = "masterRepresentations3"
...
prefixes = [('A','C')]#,('A','G')]#['P', 'G']
averageIn = ['seed']  # Averaged in all
differentGraphs = ['densityCars', 'vType']  # One by one
...
if typeRepr not in ['map', 'plot']:
    ...
    exit()
if prefixes not in [['P'], ['G'], ['P', 'G'], [('A','P'),('A','G')], [('A','P')], [('A','G')], [('A','C')]]:
                                    ...
                                    for xx in lists[x]:
                                        for avIn in averageIn:
                                            for aIn in lists[avIn]:
                                                ...
                                                params2[avIn] = aIn
                                                if params2['densityEV'] + params2['densityDiesel'] <= 1:
                                                    total_data = []
                                                    if prefix in ['P', 'G', 'C']:
